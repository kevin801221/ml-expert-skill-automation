...
class PyTorchHousingRegressor(nn.Module):
    """
    PyTorch Deep Tabular Regressor with Categorical Embeddings,
    Dense Residual Blocks, BatchNorm, and Dropout for Housing LogPrice Regression.
    """
    def __init__(self, cat_dims: Dict[str, int], emb_dims: Dict[str, int], num_dim: int, hidden_dim: int = 128, dropout_rate: float = 0.2):
        super(PyTorchHousingRegressor, self).__init__()
